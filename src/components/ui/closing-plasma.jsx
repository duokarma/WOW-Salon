...
export function ClosingPlasma({
  ...
  // Adjust container styling to be fixed background if acting as global background
  return (
    <div
      ref={containerRef}
      className={cn("relative min-h-screen w-full flex-col bg-[var(--bg-primary)] text-white", className)}
      style={style}
      {...props}
    >
      <canvas
      ...
      />
      {children && <div className="relative z-10 flex flex-col w-full">{children}</div>}
    </div>
  );
}
